...
async def _(event):
    ...
    await event.edit(f'''
𓆩هلا بيك حوبي السورس شغال
𓆩سورس تلثون • @zQQQzQz • @TLTHON
• البنگ : {ms} ✓
• التاريخ : {m9zpi} ✓
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('● ● ● @zQQQzQz ● ● ●')


@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.اعادة تشغيل"))
async def update(event):
    await event.edit(" ثواني ويشتغل / اذا صادفتك اي مشكلة تواصل ويه المطور @zQQQzQz ")
